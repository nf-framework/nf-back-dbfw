 ...
 * @param configPath {string} полный путь в конфиге до настроек тестового пользователя
 * @returns {ExecContext}
 */
function prepareContext(configPath) {
    const _config = common.getPath(config, configPath);
...
async function prepareEnv(params) {
    ...
    const _context = prepareContext(_params.configPath);
