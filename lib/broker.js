 ...
 * @param {'add'|'upd'|'del'} act - псевдодействие над таблицей
...
async function broker(connect, action, params = {}) {
    ...
    if (_act.length === 3) {
        ...
        if (act === 'mod') {
            ...
            if (unit.opt.idGenMethod === 'ext') {
                const checkPkRes = connect.query({
    ...
    if (func) {
        ...
        const { sql, params: _params } = prepareSql(unit, act, params);
        const options = {
            unitbp: unitBpCode,
            act,
            divideType: unit.opt.divideType,
            pkField: unit.pkField,
            usePrivs: unitBp.usePrivs
        };
