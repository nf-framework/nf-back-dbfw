 ...
 * @param {Object} options дополнительные настройки генерации
 * @returns {{sql: string, params: Object}}
 */
function prepareSql(unit, act, params, options = {}) {
    const _params = {};
    let sql = '', warn;
    if (act === 'add' || act === 'upd') {
        let sql_b = '',
        ...
        if (options.fields) {
            options.fields.split(';').forEach(field => {
                if (unit.columns.findIndex(f => f.name === field) !== -1) fields += `${fields ? ',' : ''}"${field}"`;
            });
        } else fields = '*';
        sql += `select ${fields} from "${unit.tableSchema}"."v4${unit.tableName}" `;
        const where = [];
        Object.keys(params).forEach(key => {
